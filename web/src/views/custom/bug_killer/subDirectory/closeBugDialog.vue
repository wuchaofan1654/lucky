<template>
  <el-dialog
    ...
    <el-form :model="bugObj" label-width="120px">
      ...
      <el-form-item label="归属RD">
        <el-select v-model="bugObj.belong_rd" filterable>
          <el-option v-for="item in rd_list" :label="item.name" :value="item.id"/>
        </el-select>
      </el-form-item>
    </el-form>
...
import {UpdateBug} from "@/views/custom/bug_killer/api";
...
export default {
  ...
  methods: {
    getRdList() {
      GetList().then(ret=>{
        this.rd_list = ret.data.data
      })
    },
    submit() {
      this.visible = false
      this.bugObj.status = 4
